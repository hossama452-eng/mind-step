...
const RPT_HEADERS: Record<string, string> = {
...
export function ReportsSection() {
  ...
  const fetchReports = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/reports/generate", { headers: RPT_HEADERS });
  ...
  const generateReport = async (type: string) => {
    ...
    try {
      const res = await fetch("/api/reports/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json", ...RPT_HEADERS },
  ...
  const exportReport = async (id: string) => {
    try {
      const res = await fetch(`/api/reports/export/${id}`, { headers: RPT_HEADERS });
  ...
  return (
    <div className="space-y-6">
      <SectionHeader title={t("reports")} description="Activity reports based on your real data." />

      <Alert className="border-info/30 bg-info/5">
        <AlertDescription className="text-xs text-muted-foreground">
          All reports are labeled as user activity tracking data — not medical diagnosis.
        </AlertDescription>
      </Alert>
      ...
      <div className="flex flex-wrap gap-2">
        {["weekly", "monthly", "focus", "habits", "energy", "comprehensive"].map((type) => (
          <Button key={type} size="sm" variant="outline" onClick={() => generateReport(type)} disabled={generating}>
            <BarChart className="size-3.5" aria-hidden />
                ...
                <CardContent className="flex items-start gap-3 p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="secondary" className="text-[10px]">Tracking data</Badge>
                    </div>
